#include "../headers/test_sub.h"

START_TEST(sub_matrix_1) {
  matrix_t A = {};
  matrix_t B = {};
  ck_assert_int_eq(sub_matrix(&A, &B, NULL), INPUT_ERR);
}
END_TEST
...
START_TEST(sub_matrix_2) {
  ...
  create_matrix(4, 4, &A);
  create_matrix(5, 5, &B);
  ck_assert_int_eq(sub_matrix(&A, &B, &C), CALC_ERR);
  remove_matrix(&A);
  remove_matrix(&B);
}
END_TEST
...
START_TEST(sub_matrix_4) {
  ...
  fill_matrix(&A, 1, 25);
  fill_matrix(&B, 1, 25);
  ck_assert_int_eq(sub_matrix(&A, &B, &C), 0);
...
START_TEST(sub_matrix_6) {
  ...
  ck_assert_int_eq(sub_matrix(&A, &B, &C), 0);
  E.matrix[0][0] = 0, E.matrix[0][1] = 2, E.matrix[0][2] = 3;
  E.matrix[1][0] = -2, E.matrix[1][1] = 4, E.matrix[1][2] = 5;
  E.matrix[2][0] = -3, E.matrix[2][1] = -4, E.matrix[2][2] = 5;
  ck_assert_int_eq(eq_matrix(&C, &E), 1);
  remove_matrix(&A);
  remove_matrix(&B);
  remove_matrix(&C);
...
Suite* suite_sub_matrix() {
  ...
  TCase* tc = tcase_create("sub-core");
